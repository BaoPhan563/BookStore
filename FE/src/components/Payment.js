...
export default function Payment({ deleteCart }) {
  ...
  const handlePayment = () => {
    ...
    const totalPrice = Math.round(
      cart.reduce((total, item) => total + item.total_price, 0),
    );
    ...
    axios
      .post(
        ...
        "http://localhost:3000/api/payment/create_payment_url",
        {
          ...
          order_details: orderDetails,
          amount: totalPrice,
          orderDescription: "Thanh toán đơn hàng sách",
          orderType: "other",
          language: "vn",
        ...
        <div className="leftCart w-3/4 pr-6">
          ...
          {cart.map((item) => (
            ...
            >
              ...
              <div className="price col-span-2 flex items-center justify-center">
                {formatPrice(fixNumber(item.price))}₫
              </div>
              ...
              <div className="totalPrice col-span-2 flex items-center justify-center">
                {formatPrice(fixNumber(item.total_price))}₫
              </div>
            </div>
          ))}
        </div>
        <div className="rightCart w-1/4 pl-6">
          <div className="shadow-2xl p-4 mb-5 h-30 rounded-lg">
            <div className="total justify-between ">
              <div className="provisionalInvoice text-lg pb-3">
                User Information
              </div>
              <div className="upperInfo flex pb-3 grid grid-cols-12">
                <div className="name pr-2 border-r-2 col-span-6">
                  {dataUser.fullname}
                </div>
