...
const BookList = ({ searchTerm }) => {
  ...
  const addToCart = async (book) => {
    if (user && user.admin) {
      toast.error("Admin không thể thêm vào giỏ hàng");
      return;
    }
    if (!user) {
  ...
  const fetchBooksByPriceRange = async (min, max) => {
    try {
      const token = getToken();
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/books/price/from${min}/to${max}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchBooksByRating = async (minRating) => {
    try {
      const token = getToken();
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/books/rating/${minRating}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  if (loading) {
    return (
      <div className="text-center py-8 text-xl text-blue-500">Loading...</div>
    );
  }
        ...
        {filteredBooks.map((book) => (
          ...
          >
            <Link to={`/book/${book._id}`}>
              <img
                src={
                  book.imageurls.length > 0
                    ? book.imageurls.find((image) => image.defaultImg)
                        ?.imageUrl || book.imageurls[0].imageUrl
              ...
              />
              <div className="p-4">
                <h2 className="text-l font-thin mb-2 text-gray-800 hover:underline text-ellipsis text-nowrap max-w-[300px] overflow-hidden">
                  {book.title}
                </h2>
                  ...
                  <button
                    ...
                    className={`${
                      user && user.admin
                        ...
                        : "text-orange-500 border-orange-400 hover:scale-105 hover:shadow-2xl"
                    } border-orange whitespace-nowrap min-w-max px-6 py-[10px] border-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 mt-6 
